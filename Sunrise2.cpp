...
Sunrise2::Sunrise2(float latitude, float longitude, float timezone){
...
// For custom angle z (in degrees, z=0 when the Sun center is at the horizon, z=6,12,18 dgr for civil, nautical and astronomical twilight)
void Sunrise2::Custom(float z){
        zenith=(90.0+z)/180.0 * 3.14159265359;
}
void Sunrise2::Actual(){
...
void Sunrise2::Nautical(){
  zenith=1.780235837034216;
}

void Sunrise2::Astronomical(){
  zenith=1.884955592153876;
}

int Sunrise2::Rise(unsigned char  month, unsigned char  day){
  return Sunrise2::Compute(month, day, 1);
}

int Sunrise2::Set(unsigned char  month, unsigned char  day){
  return Sunrise2::Compute(month, day, 0);
}

int Sunrise2::Noon(unsigned char  month, unsigned char  day){
	return Sunrise2::Compute(month, day, 2);
}

int Sunrise2::Compute(unsigned char  month, unsigned char  day, int rs) {
  float y, decl, eqt, ha;
  unsigned char a;
  int doy, minutes;
...
unsigned char Sunrise2::Hour(){
	return theHour;
}

unsigned char Sunrise2::Minute(){
	return theMinute;
}
